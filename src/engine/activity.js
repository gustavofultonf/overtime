...
export function generateProspect(year){
  ...
  const p={team:"ACADEMY",name,role,aim:talent+Math.floor(Math.random()*15),gameSense:talent+Math.floor(Math.random()*10),util:talent+Math.floor(Math.random()*10),igl:role==="IGL"?talent+15:talent-10,mentality:40+Math.floor(Math.random()*25),consistency:35+Math.floor(Math.random()*25),traits:Math.random()<0.15?["boom"]:Math.random()<0.08?["clutch"]:[],salary:5+Math.floor(Math.random()*3),contract:0,age,era:"current",form:0,fatigue:5,injury:null,
  ...
  // Hidden ceiling — how good this prospect can become. Drives the scouting
  // grade shown in the academy and caps in-house development.
  p.potential=Math.min(95,playerOvr(p)+8+Math.floor(Math.random()*26));
  return p;
}
...
export function developProspect(p){
  ...
  // A prospect stops improving once it reaches its hidden ceiling.
  if(playerOvr(p)>=(p.potential||80)) return;
...
export function autoSimWeeks(state,team,fromWeek,toWeek,facilities){
  ...
  for(let w=fromWeek;w<toWeek;w++){
    ...
    const injured=roster.some(p=>p.injury);
    const nextEv=EVENTS.find(e=>e.week>w);
    const weeksToEvent=nextEv?nextEv.week-w:99;
    // Smart activity: rest if tired or carrying injuries, train if event is close
    let act,mc=null;
    if(avgFat>70||injured) act="rest";
    ...
    if(injMsg) log.push({week:w,activity:"news",event:injMsg});
  }
  return log;
}
