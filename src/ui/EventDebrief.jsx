...
const PLACE_COLORS=["",C.gold,C.silver,C.bronze,C.dim,C.faint,C.faint,C.faint];
const CONFETTI_COLORS=[C.gold,C.acc,C.win,C.live,C.acc2];
...
export function EventDebrief({debrief,onDismiss}){
  ...
  <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.85)",display:"flex",alignItems:"center",justifyContent:"center",padding:18,zIndex:60,animation:"fadeUp .2s ease",overflow:"hidden"}}>
    ...
    <div style={{position:"relative",zIndex:2,background:C.panel,border:`1px solid ${won?C.gold+"66":C.line}`,borderRadius:14,maxWidth:580,width:"100%",maxHeight:"92vh",overflowY:"auto",animation:"popIn .42s cubic-bezier(.2,.8,.3,1)",...(won?{boxShadow:`0 0 50px -6px ${C.gold}66`}:{})}}>
      ...
      {/* Header banner */}
      <div className={won?"sheen":undefined} style={{background:won?C.gold+"14":C.panel2,borderRadius:"14px 14px 0 0",padding:"20px 22px",borderBottom:`1px solid ${C.line}`}}>
      ...
      <div style={{padding:"16px 22px"}}>
        {/* Win bonus */}
        {winBonus&&(
          <div style={{background:C.gold+"12",border:`1px solid ${C.gold}44`,borderRadius:8,padding:"8px 13px",marginBottom:14,fontFamily:mono,fontSize:11,color:C.gold}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:5,marginBottom:18}}>
          {[...playerStats].sort((a,b)=>b.rating-a.rating).map((p,i)=>{
            ...
            return(
            <div key={p.name} style={{background:i===0?C.gold+"0d":C.panel2,border:`1px solid ${i===0?C.gold+"33":C.line}`,borderRadius:8,padding:"9px 13px",display:"flex",alignItems:"center",gap:10,flexWrap:"wrap",animation:`risePop .4s ease both`,animationDelay:`${0.08+i*0.07}s`}}>
              <div style={{minWidth:90}}>
                <div style={{fontWeight:700,fontSize:13,display:"flex",alignItems:"center",gap:6}}>
                  {p.name}
                  {isMvp&&<span style={{fontFamily:mono,fontSize:8,color:C.gold,background:C.gold+"2e",padding:"2px 6px",borderRadius:3,letterSpacing:.5}}>MVP</span>}
                </div>
        ...
        <button onClick={onDismiss}
          style={{width:"100%",background:C.acc,color:C.onAcc,border:"none",borderRadius:10,padding:"14px",fontWeight:800,fontSize:15,cursor:"pointer",fontFamily:mono}}>
          CONTINUE →
        </button>
      </div>
    </div>
